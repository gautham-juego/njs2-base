...
class executor extends baseAction {
  ...
  async executeRequest(request) {
    try {
      ...
      const parameterList = await parameterProcessor.processParameter(initInstance, request);
      parameterList.forEach(parameter => {
        if(!parameter.status)
          this.setResponse('INVALID....')
        else {
          actionInstance.setMemberVariable('userObj', validatedUser); // set member variable
        }
      });
